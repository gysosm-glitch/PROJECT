...
- **팀원 모집 개설하기**: 사용자는 특정 공모전을 선택해 '팀원 모집 개설하기'를 통해 방장(Host)으로서 모집 방을 생성할 수 있습니다. 방 개설 시 필요 인원을 설정하며, 방장 프로필이 모집 방에 공개됩니다.
- **신청 및 수락 (방장 권한)**: 다른 사용자는 방장의 요약 프로필을 열람한 후 가입 신청을 합니다. 방장은 신청자의 프로필을 보고 수락/거절을 결정합니다. 남은 자리 실시간 표시되며 정원이 차면 자동 마감됩니다.
- **채팅**: 공모전 팀원 신청 수락 시 해당 팀의 **그룹 채팅방**에 자동 입장. 팀장(작성자)은 추가 멤버 초대 가능.

### 5.3 스포츠 매칭 상세
- **예약 연동**: 시설 예약 크롤링 데이터에서 '예약 가능' 슬롯을 선택하여 파트너 모집 방 개설 가능.
- **팀원 모집 개설하기**: 팀명, 종목(축구/풋살/농구/e스포츠 등), 장소, 경기일시, 원하는 수준, 필요 인원을 설정하여 모집 방을 개설합니다. 방장이 신청자를 수락/거절하는 시스템이 공모전과 동일하게 적용됩니다.
- **채팅**: 1:1 또는 다수 매치 신청이 수락되면 관련된 채팅방이 자동 생성됩니다.
