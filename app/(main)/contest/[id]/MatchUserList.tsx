...
import { ContestField, ContestProfile } from '@/types/database'
import UserContestCard from '@/components/profile/UserContestCard'
...
type ProfileWithUser = ContestProfile & { users: { nickname: string } | null }

export default function MatchUserList({ contestId, field, currentUserId }: MatchUserListProps) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      // 1. Fetch profiles where is_visible = true AND fields contains this contest's field
  ...
  if (users.length === 0) {
    return (
      <div className="glass-card py-16 text-center">
        <p className="text-gray-400 font-medium">아직 이 분야에 관심 있는 유저가 없습니다.</p>
