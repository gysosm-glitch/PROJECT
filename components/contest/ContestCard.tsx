...
function DeadlineBadge({ endDate }: { endDate: string }) {
  ...
  if (days < 0) return <span className="badge-red">ÎßàÍ∞ê</span>
  if (days === 0) return <span className="badge-red animate-pulse">?§ÎäòÎßàÍ∞ê</span>
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        {contest.prize && (
          <div className="flex items-center gap-1.5 text-accent-400 text-xs font-medium">
            <span className="text-accent-500">?í∞</span>
        ...
        <div className="mt-auto space-y-2 pt-3 border-t border-surface-border">
          ...
          {contest.max_participants && (
            <div className="text-xs text-gray-500">
              <span className="text-accent-400 font-medium">ÏµúÎ? {contest.max_participants}Î™?/span>
            </div>
          )}
        </div>
      </div>
